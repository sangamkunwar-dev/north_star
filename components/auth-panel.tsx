...
export function AuthPanel({ onNotice }: { onNotice: (message: string) => void }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (!supabase) {
      onNotice('Authentication is not configured yet. Add the Supabase URL and publishable key.')
      return
    }
    ...
    try {
      if (mode === 'signup') {
        ...
        if (data.user && !data.session) {
          ...
          onNotice('Account created. Twilio Verify sent an SMS code to your phone.')
        } else {
      ...
      if (errCode === 'phone_provider_disabled' || message.includes('phone provider') || message.includes('phone signups are disabled')) {
        onNotice('Phone auth is disabled. Enable Phone Auth and Twilio Verify in Supabase Providers.')
      } else if (errCode === 'signup_disabled' || message.includes('signups not allowed')) {
        onNotice('Signups are disabled in Supabase Auth settings.')
      } else if (errCode === 'sms_send_failed' || message.includes('sms') || message.includes('twilio') || message.includes('verify')) {
        onNotice('Failed to send SMS via Twilio Verify. Check your Service SID (VA...) in Supabase.')
      } else if (errCode === 'user_already_exists' || message.includes('already registered') || message.includes('already exists')) {
        onNotice('This phone number is already registered. Please log in.')
        ...
        onNotice('Enter a valid phone number in international format (+977...).')
      } else if (message.includes('rate')) {
        onNotice('Too many verification attempts. Please wait a moment and try again.')
      } else {
        onNotice(error?.message || 'Authentication failed. Please check your settings.')
      }
    } finally {
      setBusy(false)
    }
  ...
  async function verify() {
    ...
    if (!supabase) {
      onNotice('Authentication is not configured yet.')
      return
    }
    ...
    try {
      ...
      if (error) throw error
      onNotice('Phone verified successfully! Your account is ready.')
      setNeedsVerification(false)
    } catch {
      onNotice('Invalid or expired OTP code.')
    } finally {
      setBusy(false)
    }
  }
  ...
  async function resendCode() {
    ...
    try {
      ...
      if (error) throw error
      onNotice('A new Twilio Verify code was sent to your phone.')
    } catch {
      onNotice('Could not resend the verification code. Try again in a minute.')
    } finally {
      setBusy(false)
    }
  }
  ...
  async function google() {
    ...
    if (error) onNotice('Google sign-in failed. Please try again.')
  }
    ...
    <div className="w-full max-w-md rounded-3xl border border-border bg-card p-7 shadow-2xl shadow-primary/10">
      <div className="mb-7">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          {needsVerification ? 'Enter the code sent via Twilio Verify to continue.' : 'Plan, write, and publish from one focused workspace.'}
        </p>
      </div>

      {needsVerification ? (
        <div className="space-y-4">
          <label className="label" htmlFor="otp">SMS verification code</label>
          <input
            id="otp"
            value={code}
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <button
            type="button"
            onClick={google}
            className="flex w-full items-center justify-center gap-3 rounded-xl border border-border px-4 py-3 text-sm font-bold hover:bg-muted"
          >
            <GoogleMark />
