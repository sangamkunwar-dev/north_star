...
function SubscriptionDashboard({ plan, setPlan, onNotice }: { plan: 'free' | 'creator' | 'studio'; setPlan: (plan: 'free' | 'creator' | 'studio') => void; onNotice: (message: string) => void }) {
  ...
  function choose(id: 'free' | 'creator' | 'studio') { if (id === 'free') { setPlan(id); setPurchasedAt(null); setCheckoutState('idle'); onNotice('You are using the Free plan.'); return }; setPlan(id); setCheckoutState('pending'); setPurchasedAt(new Date().toISOString()); onNotice(`${id === 'creator' ? 'Creator' : 'Studio'} purchase recorded as pending verification.`) }
