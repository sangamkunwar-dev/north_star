...
function SubscriptionDashboard({ plan, setPlan, onNotice }: { plan: 'free' | 'creator' | 'studio'; setPlan: (plan: 'free' | 'creator' | 'studio') => void; onNotice: (message: string) => void }) {
  ...
  function choose(id: 'free' | 'creator' | 'studio') { setPlan(id); if (id === 'free') { setPurchasedAt(null); setCheckoutState('idle'); setShowPayment(false); onNotice('You are using the Free plan.'); return }; setShowPayment(true); setPaymentMethod('card'); requestAnimationFrame(() => paymentRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })); }
