...
import { BarChart3, CalendarClock, Check, FileText, ImagePlus, LayoutDashboard, LifeBuoy, LogOut, Menu, PenLine, Plus, QrCode, Settings2, Sparkles, Trash2, Video, X } from 'lucide-react'
...
function SubscriptionDashboard({ plan, setPlan, onNotice }: { plan: 'free' | 'creator' | 'studio'; setPlan: (plan: 'free' | 'creator' | 'studio') => void; onNotice: (message: string) => void }) {
  const plans = [
    { id: 'free', name: 'Free', price: 'रु 0', features: ['5 posts / month', '3 AI generations', '1 destination', 'Basic history', 'Community support'] },
    { id: 'creator', name: 'Creator', price: 'रु 999 / month', features: ['50 posts / month', 'Unlimited AI writing', '5 destinations', 'Scheduling + analytics', 'Priority support'] },
    { id: 'studio', name: 'Studio', price: 'रु 2,499 / month', features: ['Unlimited posts', 'Unlimited AI writing', 'Unlimited destinations', 'Team workflows + priority support', 'Advanced reporting'] },
  ] as const
  function choose(id: 'free' | 'creator' | 'studio') {
    setPlan(id)
    if (id === 'free') { onNotice('You are using the Free plan.'); return }
    window.location.href = `/payment?plan=${id}`
  }
  return <div className="mt-8 max-w-6xl space-y-6">
    <div>
      <p className="text-sm font-bold uppercase tracking-widest text-primary">Plans & billing</p>
      <h2 className="mt-2 text-3xl font-semibold tracking-tight">Choose the workspace that fits your rhythm.</h2>
      <p className="mt-2 text-sm text-muted-foreground">Select a plan to continue to secure payment. Billing details appear after selection.</p>
    </div>
    <div className="grid gap-4 lg:grid-cols-3">{plans.map((item) => <div key={item.id} className={`rounded-2xl border bg-card p-6 ${plan === item.id ? 'border-primary shadow-lg shadow-primary/10' : 'border-border'}`}>
      <div className="flex items-center justify-between gap-3"><div><h3 className="text-lg font-semibold">{item.name}</h3><p className="mt-2 text-2xl font-bold">{item.price}</p></div>{plan === item.id && <span className="rounded-full bg-primary/10 px-2 py-1 text-xs font-bold text-primary">Current</span>}</div>
      <ul className="mt-6 flex flex-col gap-3">{item.features.map((feature) => <li key={feature} className="flex gap-2 text-sm text-muted-foreground"><Check size={16} className="mt-0.5 shrink-0 text-primary" />{feature}</li>)}</ul>
      <button type="button" onClick={() => choose(item.id)} className="mt-7 w-full rounded-xl bg-primary px-4 py-3 text-sm font-bold text-primary-foreground hover:opacity-90">{item.id === plan ? 'Selected plan' : item.id === 'free' ? 'Use Free plan' : `Choose ${item.name}`}</button>
    </div>)}</div>
  </div>
}

function LegacyPaymentSection({ plan, plans, checkoutUrl, qrUrl, paymentMethod, setPaymentMethod, onSubmit }: { plan: 'creator' | 'studio'; plans: readonly { id: 'free' | 'creator' | 'studio'; name: string; price: string; features: readonly string[] }[]; checkoutUrl: string; qrUrl: string; paymentMethod: 'card' | 'qr'; setPaymentMethod: (method: 'card' | 'qr') => void; onSubmit: (event: React.FormEvent<HTMLFormElement>) => void }) {
  return <div id="payment-section" className="scroll-mt-8 rounded-2xl border border-border bg-card p-6 sm:p-8"><p className="text-sm font-bold uppercase tracking-widest text-primary">Secure checkout</p><h2 className="mt-2 text-2xl font-semibold">Pay for the {plan} plan</h2><p className="mt-2 text-sm text-muted-foreground">Complete payment below. Your plan activates after payment verification.</p><div className="mt-6 flex flex-wrap gap-3"><button type="button" onClick={() => setPaymentMethod('card')} className={`rounded-xl px-5 py-3 text-sm font-bold ${paymentMethod === 'card' ? 'bg-primary text-primary-foreground' : 'border border-border text-muted-foreground'}`}>Card</button><button type="button" onClick={() => setPaymentMethod('qr')} className={`rounded-xl px-5 py-3 text-sm font-bold ${paymentMethod === 'qr' ? 'bg-primary text-primary-foreground' : 'border border-border text-muted-foreground'}`}>QR code</button></div>{paymentMethod === 'card' ? <form onSubmit={onSubmit} className="mt-7 max-w-xl"><p className="text-sm font-semibold">Card information <span className="float-right text-xs text-muted-foreground">VISA · Mastercard</span></p><div className="mt-4 flex flex-col gap-4"><input required autoComplete="cc-name" className="field" placeholder="Card Holder Name*" /><input required inputMode="numeric" autoComplete="cc-number" className="field" placeholder="Enter card number" /><div className="grid gap-4 sm:grid-cols-2"><input required autoComplete="cc-exp" className="field" placeholder="MM / YY" /><input required inputMode="numeric" autoComplete="cc-csc" className="field" placeholder="CVV" /></div></div><button type="submit" className="mt-6 w-full rounded-xl bg-primary px-5 py-3 text-sm font-bold text-primary-foreground">Pay {plans.find((item) => item.id === plan)?.price}</button><p className="mt-3 text-xs text-muted-foreground">Demo checkout only. Card details are not stored.</p></form> : <div className="mt-7 flex flex-col items-start gap-4 sm:flex-row sm:items-center"><a href={checkoutUrl} target="_blank" rel="noreferrer" className="rounded-lg bg-background p-2"><img src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-vReucrepROLzoZ8fKDALzfz4ZGZ2WG.png" alt="Khalti QR code for Muna Kunwar" className="max-h-96 w-auto rounded-xl object-contain" /></a><p className="max-w-sm text-sm text-muted-foreground">Scan to open the {plan} checkout on another device.</p></div>}</div>
}
