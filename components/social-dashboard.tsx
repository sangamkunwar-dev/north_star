...
function SubscriptionDashboard({ plan, setPlan, onNotice }: { plan: 'free' | 'creator' | 'studio'; setPlan: (plan: 'free' | 'creator' | 'studio') => void; onNotice: (message: string) => void }) {
  ...
  function choose(id: 'free' | 'creator' | 'studio') { setPlan(id); if (id === 'free') { setSubmittedAt(null); setCheckoutState('idle'); onNotice('You are using the Free plan.'); return }; window.location.href = `/payment?plan=${id}` }
