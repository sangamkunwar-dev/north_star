...
function Editor(p: any) {
  ...
  const canContinue = Boolean(p.topic.trim()) && p.selectedTargets.length > 0 && p.channels.length > 0
  ...
  function toggleTarget(id: string) {
    ...
    p.setSelectedTargets(next)
    p.setChannels(targetChannels(next))
  }
  function nextStep() {
    if (!p.topic.trim()) { p.setNotice('Describe what you want to share first.'); return }
    if (!p.targets.length) { p.setNotice('Connect a Facebook Page or Instagram account before continuing.'); return }
    if (!p.selectedTargets.length) { p.setNotice('Select at least one Page or account.'); return }
    setStep(2)
  }
  return <div className="mt-8 max-w-4xl">
    <div className="mb-6 flex items-center gap-3" aria-label="Create post progress"><div className={`flex items-center gap-2 text-sm font-semibold ${step === 1 ? 'text-primary' : 'text-muted-foreground'}`}><span className="grid size-8 place-items-center rounded-full bg-primary text-primary-foreground">1</span>Plan</div><div className="h-px flex-1 bg-border"/><div className={`flex items-center gap-2 text-sm font-semibold ${step === 2 ? 'text-primary' : 'text-muted-foreground'}`}><span className={`grid size-8 place-items-center rounded-full ${step === 2 ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>2</span>Create</div></div>
    {step === 1 ? <div className="grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
      <Card title="Step 1 · Plan your post"><label className="label">What do you want to share?</label><textarea value={p.topic} onChange={(e: any) => p.setTopic(e.target.value)} rows={5} className="field mt-2" placeholder="Describe your announcement, idea, or moment..."/><label className="label mt-5 block">Mention users or accounts</label><input value={p.mentions} onChange={(e: any) => p.setMentions(e.target.value)} className="field mt-2" placeholder="@northstar or partner accounts"/><div className="mt-6 flex items-center justify-between gap-4"><div><p className="text-sm font-semibold">Publishing integrations</p><p className="mt-1 text-xs text-muted-foreground">Connect channels to unlock Pages and accounts.</p></div><div className="flex gap-2"><button type="button" onClick={() => p.connect('facebook')} className="rounded-lg border border-border px-3 py-2 text-xs font-semibold hover:bg-muted">{p.connections.facebook ? 'Facebook connected' : 'Connect Facebook'}</button><button type="button" onClick={() => p.connect('instagram')} className="rounded-lg border border-border px-3 py-2 text-xs font-semibold hover:bg-muted">{p.connections.instagram ? 'Instagram connected' : 'Connect Instagram'}</button></div></div></Card>
      <Card title="Select Pages and accounts"><p className="text-sm text-muted-foreground">Choose where this post should be published.</p>{p.targets.length ? <div className="mt-4 flex flex-col gap-2">{p.targets.map((target: SocialTarget) => <label key={target.id} className="flex cursor-pointer items-center gap-3 rounded-xl border border-border p-3 hover:bg-muted"><input type="checkbox" checked={p.selectedTargets.includes(target.id)} onChange={() => toggleTarget(target.id)} className="size-4 accent-primary"/><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{target.name}</span><span className="text-xs capitalize text-muted-foreground">{target.channel}</span></span>{p.selectedTargets.includes(target.id) && <Check size={16} className="text-primary"/>}</label>)}</div> : <div className="mt-5 rounded-xl bg-muted p-4 text-sm text-muted-foreground">No Pages or accounts connected yet.</div>}<button type="button" onClick={nextStep} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-bold text-primary-foreground">Next <span aria-hidden="true">→</span></button></Card>
    </div> : <div className="grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
      <Card title="Step 2 · Create your content"><label className="label">Add photo or video</label><input type="file" accept="image/*,video/*" onChange={(e: any) => { const file = e.target.files?.[0]; if (!file) return; p.setMedia({ file, fileName: file.name, url: URL.createObjectURL(file), type: file.type.startsWith('video') ? 'video' : 'image', aspect: 'landscape' }) }} className="field mt-2"/><label className="label mt-4 block">Or paste a public media URL</label><input value={p.media?.file ? '' : (p.media?.url || '')} onChange={(e: any) => p.setMedia(e.target.value ? { url: e.target.value, fileName: 'linked-media', type: 'image', aspect: 'landscape' } : null)} className="field mt-2" placeholder="https://..."/>{p.media && <div className="mt-4 flex items-center gap-3 rounded-xl border border-border p-3">{p.media.type === 'video' ? <Video size={20} className="text-primary"/> : <img src={p.media.url} alt="Selected media preview" className="size-14 rounded-lg object-cover"/>}<span className="min-w-0 flex-1 truncate text-sm">{p.media.fileName}</span><button type="button" onClick={() => p.setMedia(null)} aria-label="Remove media"><X size={18}/></button></div>}<button type="button" onClick={p.generate} disabled={p.busy} className="mt-5 flex items-center gap-2 rounded-xl border border-primary/30 px-4 py-3 text-sm font-semibold text-primary hover:bg-primary/10"><Sparkles size={17}/> {p.busy ? 'Writing...' : 'Generate caption with AI'}</button><label className="label mt-5 block">Caption</label><textarea value={p.caption} onChange={(e: any) => p.setCaption(e.target.value)} rows={6} className="field mt-2" placeholder="Write the main caption..."/></Card>
      <Card title="Polish and publish"><label className="label">Hashtags</label><input value={p.hashtags} onChange={(e: any) => p.setHashtags(e.target.value)} className="field mt-2" placeholder="#northstar #community"/><label className="label mt-4 block">Call to action</label><input value={p.cta} onChange={(e: any) => p.setCta(e.target.value)} className="field mt-2" placeholder="Learn more, join us, or shop now"/><label className="label mt-4 block">Schedule in Nepal time (optional)</label><input type="datetime-local" value={p.schedule} onChange={(e: any) => p.setSchedule(e.target.value)} className="field mt-2"/><div className="mt-6 flex flex-col gap-3 sm:flex-row"><button type="button" onClick={() => setStep(1)} className="rounded-xl border border-border px-4 py-3 text-sm font-semibold hover:bg-muted">Back</button><button type="button" onClick={p.savePost} disabled={p.busy} className="flex-1 rounded-xl bg-primary px-4 py-3 text-sm font-bold text-primary-foreground">{p.busy ? 'Saving...' : p.schedule ? 'Schedule post' : 'Publish post'}</button></div></Card>
    </div>}
  </div>
}
